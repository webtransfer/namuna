...
$full_www_path 		= "";				# Полный WWW (http/https) путь к папке, где лежат скрипты биллинга с символом / в конце. Пример: https://www.myhost.net/
...
#$_CFG['ORDER_V2_BY_DEFAULT'] = true;			# Использовать версию 2 скрипта оформления заказа по умолчанию.

#$_CFG['PAY_KVITANCIYA_DISABLE_HTML'] = true;		# Отключить HTML-кнопку для квитанций для физ. лиц.
#$_CFG['PAY_KVITANCIYA_DISABLE_PDF'] = true;		# Отключить PDF-кнопку для квитанций для физ. лиц.
#$_CFG['PAY_SCHET_DISABLE_HTML'] = true;		# Отключить HTML-кнопку для счетов для юр. лиц.
#$_CFG['PAY_SCHET_DISABLE_PDF'] = true;			# Отключить PDF-кнопку для счетов для юр. лиц.
...
$BILLING_USAGE		= false;			# Отображение времени выполнения скриптов и используемой ими памяти.
...
#$_CFG['SOLUSVM_HOSTNAME_TPL'] = "{login}.{id}.vm";	# Шаблон для hostname виртуальной машины, создаваемой в SolusVM.
#$_CFG['VIRTUALIZOR_HOSTNAME_TPL'] = "{login}.{id}.vm";	# Шаблон для hostname виртуальной машины, создаваемой в Virtualizor.
#$_CFG['VMMANAGER_HOSTNAME_TPL'] = "{login}.{id}.vm";	# Шаблон для hostname виртуальной машины, создаваемой в VMManager KVM.
#$_CFG['VEMANAGER_HOSTNAME_TPL'] = "{login}.{id}.vm";	# Шаблон для hostname виртуальной машины, создаваемой в VMManager OVZ.
...
$VMMANAGER_DEFAULT_OS 	= "Centos-6-amd64-ispmgr5";	# OS по умолчанию (если клиент не указал OS при заказе) для VMManager KVM.
$VEMANAGER_DEFAULT_OS 	= "centos-6-x86-minimal";	# OS по умолчанию (если клиент не указал OS при заказе) для VMManager OVZ.
